package com.example.warehouseCounter.ui.activities.orderRequest
...
import com.example.warehouseCounter.data.ktor.v2.dto.order.Log
...
import com.example.warehouseCounter.data.room.dao.orderRequest.LogCoroutines
import com.example.warehouseCounter.data.room.dao.orderRequest.OrderRequestCoroutines
import com.example.warehouseCounter.databinding.OrderRequestActivityBothPanelsCollapsedBinding
import com.example.warehouseCounter.misc.CurrentUser
import com.example.warehouseCounter.misc.Statics
import com.example.warehouseCounter.misc.Statics.Companion.DATE_FORMAT
...
import com.example.warehouseCounter.misc.objects.status.ConfirmStatus.CREATOR.confirm
import com.example.warehouseCounter.misc.objects.status.ConfirmStatus.CREATOR.modify
...
import com.example.warehouseCounter.scanners.scanCode.GetOrderRequestContentFromCode
import com.example.warehouseCounter.scanners.scanCode.GetOrderRequestContentFromCode.OrderRequestContentResult
import com.example.warehouseCounter.scanners.scanCode.GetResultFromCode
import com.example.warehouseCounter.ui.activities.common.EnterCodeActivity
import com.example.warehouseCounter.ui.activities.common.MultiplierSelectActivity
import com.example.warehouseCounter.ui.activities.common.QtySelectorActivity
import com.example.warehouseCounter.ui.activities.item.ItemSelectActivity
import com.example.warehouseCounter.ui.activities.log.LogContentActivity
...
class OrderRequestContentActivity : AppCompatActivity(), OrcAdapter.DataSetChangedListener,
    ...
    private fun saveBundleValues(b: Bundle) {
        if (adapter != null) {
            ...
            b.putStringArrayList("checkedHashArray", adapter?.checkedHashArray ?: ArrayList())
    ...
    private fun setMultiplierButtonText() {
        runOnUiThread {
            binding.multiplierButton.text = String.format(
                context.getString(R.string.multiplier_x), settingsVm.scanMultiplier.toString()
            )
        }
    }
    ...
    private fun saveTempOrder(onFinish: () -> Unit = {}) {
        ...
        OrderRequestCoroutines.update(
            ...
            onEvent = {
                if (!SnackBarType.SUCCESS.equals(it.snackBarType)) {
    ...
    override fun scannerCompleted(scanCode: String) {
        ...
        GetOrderRequestContentFromCode(
            code = code,
            list = fullList,
            onEvent = { it2 -> showMessage(it2.text, it2.snackBarType) },
            onFinish = {
    ...
    private fun showMessage(msg: String, type: Int) {
        ...
        if (ERROR.equals(type)) android.util.Log.e(javaClass.simpleName, msg)
    ...
    private fun setQty(orc: OrderRequestContent) {
        if (tempLotCode != null && tempQty == null) showMessage(
    ...
    private fun checkLotEnabled(orc: OrderRequestContent) {
        ...
        val lotCode = tempLotCode ?: orc.lotCode
