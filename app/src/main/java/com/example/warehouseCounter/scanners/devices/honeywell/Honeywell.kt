package com.example.warehouseCounter.scanners.devices.honeywell
...
class Honeywell(private val activity: AppCompatActivity) : Scanner() {
    ...
    internal object Constants {
         ...
         */
        const val ACTION_BARCODE_DATA = "com.example.warehousecounter.intent.action.BARCODE"
    ...
    private fun loadProperties() {
        ...
        properties?.putBoolean(Constants.PROPERTY_PDF_417_ENABLED, settingsVm.symbologyPDF417)
        properties?.putBoolean(Constants.PROPERTY_AZTEC_ENABLED, settingsVm.symbologyAztec)
        properties?.putBoolean(Constants.PROPERTY_QR_CODE_ENABLED, settingsVm.symbologyQRCode)
        properties?.putBoolean(Constants.PROPERTY_CODABAR_ENABLED, settingsVm.symbologyCODABAR)
        properties?.putBoolean(Constants.PROPERTY_CODE_128_ENABLED, settingsVm.symbologyCode128)
        properties?.putBoolean(Constants.PROPERTY_CODE_39_ENABLED, settingsVm.symbologyCode39)
        properties?.putBoolean(Constants.PROPERTY_CODE_93_ENABLED, settingsVm.symbologyCode93)
        properties?.putBoolean(
            Constants.PROPERTY_DATAMATRIX_ENABLED,
            settingsVm.symbologyDataMatrix
        )
        properties?.putBoolean(Constants.PROPERTY_EAN_13_ENABLED, settingsVm.symbologyEAN13)
        properties?.putBoolean(Constants.PROPERTY_EAN_8_ENABLED, settingsVm.symbologyEAN8)
        properties?.putBoolean(Constants.PROPERTY_MAXICODE_ENABLED, settingsVm.symbologyMaxiCode)
        properties?.putBoolean(Constants.PROPERTY_RSS_ENABLED, settingsVm.symbologyRSS14)
        properties?.putBoolean(
            Constants.PROPERTY_RSS_EXPANDED_ENABLED,
            settingsVm.symbologyRSSExpanded
        )
        properties?.putBoolean(Constants.PROPERTY_UPC_A_ENABLE, settingsVm.symbologyUPCA)
        properties?.putBoolean(Constants.PROPERTY_UPC_E_ENABLED, settingsVm.symbologyUPCE)
    ...
    private fun resumeScanner(): Boolean {
        ...
        try {
            ...
            properties?.putBoolean(Constants.PROPERTY_DATA_PROCESSOR_DATA_INTENT, true)
    ...
    private fun registerReceiver() {
        Log.v(javaClass.simpleName, "Registering receiver (${activityName})")
        try {
