package com.example.warehouseCounter.scanners.devices.nfc
...
object Nfc {
    ...
    fun setupNFCReader(targetActivity: AppCompatActivity?) {
        ...
        if (mNfcAdapter == null) {
            ...
            Log.d(
                javaClass.simpleName, context.getString(R.string.this_device_doesnt_support_NFC)
            )
            return
        }

        if (!mNfcAdapter!!.isEnabled) {
            Log.d(javaClass.simpleName, context.getString(R.string.NFC_is_disabled))
        } else {
            Log.d(javaClass.simpleName, context.getString(R.string.NFC_is_enabled))
        }
    ...
    fun nfcHandleIntent(intent: Intent, targetActivity: AppCompatActivity?) {
        ...
        if (NfcAdapter.ACTION_TECH_DISCOVERED == action) {
            ...
            when (targetActivity) {
                is ItemSelectActivity -> targetActivity.scannerCompleted(id)
            }
        }
    }
