package com.example.warehouseCounter.scanners.devices.floatingCamera
...
import com.example.warehouseCounter.databinding.FloatingCameraActivityBinding
...
import com.example.warehouseCounter.ui.views.ScaleImageView
...
class FloatingCameraBarcode(private var activity: AppCompatActivity) : BarcodeCallback,
    ...
    private lateinit var portraitLoc: IntArray
    private var portraitWidth: Int = 0
    private var portraitHeight: Int = 0
    private lateinit var landscapeLoc: IntArray
    private var landscapeWidth: Int = 0
    ...
    private fun onCreate() {
        ...
        EasyFloat.with(activity)
            ...
            .setLocation(
                if (getOrientation == Configuration.ORIENTATION_PORTRAIT) portraitLoc[0] else landscapeLoc[0],
                if (getOrientation == Configuration.ORIENTATION_PORTRAIT) portraitLoc[1] else landscapeLoc[1]
            )
            ...
            .registerCallback {
                ...
                dragEnd {
                    ...
                    if (getOrientation == Configuration.ORIENTATION_PORTRAIT) {
                        it.getLocationOnScreen(portraitLoc)
                        //flCameraPortraitLoc[1] = flCameraPortraitLoc[1] - allBarHeight
                    } else {
                        it.getLocationOnScreen(landscapeLoc)
        ...
        floatWindowDisplayed = EasyFloat.isShow(activityName)
    }
    ...
    fun onDestroy() {
        filterTimer?.cancel()
        saveValues()
        EasyFloat.dismiss(activityName, true)
    ...
    private fun setLayout() {
        ...
        val params = FrameLayout.LayoutParams(
            if (getOrientation == Configuration.ORIENTATION_PORTRAIT) portraitWidth
            else landscapeWidth,
            if (getOrientation == Configuration.ORIENTATION_PORTRAIT) portraitHeight
            else landscapeHeight
        )
        ...
        ivScale.onScaledListener = object : ScaleImageView.OnScaledListener {
            override fun onScaled(x: Float, y: Float, event: MotionEvent) {
                params.width = min(max(params.width + x.toInt(), minWidth), screenWidth)
                params.height = min(max(params.height + y.toInt(), minHeight), screenHeight)
        ...
        // ivScale.setOnTouchListener { _, event ->
        //     when (event.action) {
    ...
    private fun setScale(params: LayoutParams) {
        // Guardar el tamaño actual
        if (getOrientation == Configuration.ORIENTATION_PORTRAIT) {
            portraitWidth = params.width
            portraitHeight = params.height
        } else {
            landscapeWidth = params.width
            landscapeHeight = params.height
        }

        // Actualice el tamaño del diseño raíz xml
        binding.rlContent.layoutParams = params
    ...
    private fun getDefaultHeight(): Int {
        return if (getOrientation == Configuration.ORIENTATION_PORTRAIT) {
            settingsRepository.flCameraPortraitHeight.default as Int
        } else {
            settingsRepository.flCameraLandscapeHeight.default as Int
        }
    }

    private fun getDefaultWidth(): Int {
        return if (getOrientation == Configuration.ORIENTATION_PORTRAIT) {
            settingsRepository.flCameraPortraitWidth.default as Int
        } else {
            settingsRepository.flCameraLandscapeHeight.default as Int
        }
    }
    ...
    private fun loadValues() {
        ...
        minWidth = settingsRepository.flCameraPortraitWidth.default as Int
        minHeight = settingsRepository.flCameraPortraitHeight.default as Int

        portraitLoc = intArrayOf(settingsVm.flCameraPortraitLocX, settingsVm.flCameraPortraitLocY)
        portraitWidth = min(settingsVm.flCameraPortraitWidth, screenWidth)
        portraitHeight = min(settingsVm.flCameraPortraitHeight, screenHeight)
        landscapeLoc = intArrayOf(settingsVm.flCameraLandscapeLocX, settingsVm.flCameraLandscapeLocY)
        landscapeWidth = min(settingsVm.flCameraLandscapeWidth, screenWidth)
        landscapeHeight = min(settingsVm.flCameraLandscapeHeight, screenHeight)

        continuousOn = settingsVm.flCameraContinuousMode
        filterRepeatedReads = settingsVm.flCameraFilterRepeatedReads
    }
    ...
    private fun saveValues() {
        ...
        settingsVm.flCameraPortraitLocX = portraitLoc[0]
        settingsVm.flCameraPortraitLocY = portraitLoc[1]
        settingsVm.flCameraPortraitWidth = portraitWidth
        settingsVm.flCameraPortraitHeight = portraitHeight
        settingsVm.flCameraLandscapeLocX = landscapeLoc[0]
        settingsVm.flCameraLandscapeLocY = landscapeLoc[1]
        settingsVm.flCameraLandscapeWidth = landscapeWidth
        settingsVm.flCameraLandscapeHeight = landscapeHeight
    ...
    private val resultForCameraPermission =
        activity.registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
            ...
            if (!isGranted) {
                showMessage(activity.window, context.getString(R.string.app_dont_have_necessary_permissions), ERROR)
            } else {
                checkFloatingPermission()
            }
        }
    ...
    fun hideWindow() {
        if (floatWindowCreated && EasyFloat.isShow(activityName)) EasyFloat.hide(
            activityName
        )
    }

    private fun showWindow() {
        if (floatWindowCreated && !EasyFloat.isShow(activityName)) EasyFloat.show(
            activityName
        )
    }
    ...
    fun toggleWindowVisibility() {
        if (floatWindowCreated) {
            if (EasyFloat.isShow(activityName)) EasyFloat.hide(activityName)
            else EasyFloat.show(activityName)
        ...
        try {
            ...
            showMessage(activity.window, "Error: ${ex.message}", ERROR)
            ex.message
        }
    }
    ...
    private fun createResult() {
        ...
        if (settingsVm.symbologyPDF417) formats.add(BarcodeFormat.PDF_417)
        if (settingsVm.symbologyAztec) formats.add(BarcodeFormat.AZTEC)
        if (settingsVm.symbologyQRCode) formats.add(BarcodeFormat.QR_CODE)
        if (settingsVm.symbologyCODABAR) formats.add(BarcodeFormat.CODABAR)
        if (settingsVm.symbologyCode128) formats.add(BarcodeFormat.CODE_128)
        if (settingsVm.symbologyCode39) formats.add(BarcodeFormat.CODE_39)
        if (settingsVm.symbologyCode93) formats.add(BarcodeFormat.CODE_93)
        if (settingsVm.symbologyDataMatrix) formats.add(BarcodeFormat.DATA_MATRIX)
        if (settingsVm.symbologyEAN13) formats.add(BarcodeFormat.EAN_13)
        if (settingsVm.symbologyEAN8) formats.add(BarcodeFormat.EAN_8)
        if (settingsVm.symbologyMaxiCode) formats.add(BarcodeFormat.MAXICODE)
        if (settingsVm.symbologyRSS14) formats.add(BarcodeFormat.RSS_14)
        if (settingsVm.symbologyRSSExpanded) formats.add(BarcodeFormat.RSS_EXPANDED)
        if (settingsVm.symbologyUPCA) formats.add(BarcodeFormat.UPC_A)
        if (settingsVm.symbologyUPCE) formats.add(BarcodeFormat.UPC_E)
    ...
    private val isFilterRunning: AtomicBoolean = AtomicBoolean(false)
    ...
    override fun barcodeResult(result: BarcodeResult) {
        ...
        if (lastScannedCode.contains("config") ||
            lastScannedCode.contains("log_user_name") ||
            lastScannedCode.contains("log_password")
        ) return
        else binding.lastCodeScanned.text = lastScannedCode

        // Bloquear nuevamente si está en Modo de disparo simple
        if (binding.barcodeView.barcodeView.decodeMode == SINGLE) {
            binding.messageTextView.text = context.getString(R.string.touch_the_image_to_start_reading)
