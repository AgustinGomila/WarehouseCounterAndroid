...
import com.example.warehouseCounter.data.ktor.v1.dto.ptlOrder.ApiResponse.Companion.RESULT_OK
...
class DetachOrderToLocation(
    ...
    private suspend fun suspendFunction() = withContext(Dispatchers.IO) {
        ...
        apiServiceV1.detachPtlOrderToLocation(body = body, callback = {
    ...
    private fun sendEvent(event: SnackBarEventData) {
        ...
        if (event.snackBarType in getFinish()) {
            onFinish.invoke(SnackBarType.SUCCESS.equals(event.snackBarType))
        }
    }
}
