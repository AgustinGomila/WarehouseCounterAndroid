package com.dacosys.warehouseCounter.ktor.v1.impl
...
import com.dacosys.warehouseCounter.ktor.v1.dto.database.DatabaseDataIntermediate
...
import com.dacosys.warehouseCounter.ktor.v1.dto.ptlOrder.ApiResponse.Companion.RESULT_ERROR
...
import com.dacosys.warehouseCounter.ktor.v1.service.APIService
...
class APIServiceImpl : APIService {
    ...
    override suspend fun getDbLocation(
        version: String,
        callback: (ArrayList<DatabaseData>) -> Unit
    ) {
        val url = "$apiUrl/api/database/location$version"
        var res: ArrayList<DatabaseData> = ArrayList()
        val result: String
        try {
            result = httpClient.post {
                url(url)
            ...
            val inter =
                json.decodeFromString<DatabaseDataIntermediate>(result)
