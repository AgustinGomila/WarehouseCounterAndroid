package com.dacosys.warehouseCounter.data.sync
        ...
        private fun setConfigPanel(
            ...
            for (pack in packArray) {
                ...
                for ((key, value) in customOptJsonObj) {
                    if (key == com.dacosys.warehouseCounter.data.ktor.v1.dto.clientPackage.Package.IC_USER_KEY) {
                        icUser = when (value) {
                            is JsonPrimitive -> value.content
                            else -> value.toString()
                        }
                    } else if (key == com.dacosys.warehouseCounter.data.ktor.v1.dto.clientPackage.Package.IC_PASSWORD_KEY) {
                        icPass = when (value) {
                            is JsonPrimitive -> value.content
                            else -> value.toString()
                        }
                ...
                if (productId == Statics.APP_VERSION_ID) {
                    ...
                    // User/Pass for ImageControl come from the client main package configuration
                    sv.icUser = icUser
                    sv.icPass = icPass
