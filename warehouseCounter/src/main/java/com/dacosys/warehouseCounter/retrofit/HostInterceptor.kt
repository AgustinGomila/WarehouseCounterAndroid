...
class HostInterceptor : Interceptor {
    ...
    private val dummyResponse: Response = Response.Builder().request(
        Request.Builder().url(DynamicRetrofit.getUrl()).build()
    ).protocol(Protocol.HTTP_2).code(404)
        .message(WarehouseCounterApp.context.getString(R.string.invalid_url)).body(
            ResponseBody.create(
