...
import com.dacosys.warehouseCounter.dto.ptlOrder.ApiResponse
...
class DetachOrderToLocation(
    ...
    private suspend fun suspendFunction() = withContext(Dispatchers.IO) {
        ...
        val tempInst = apiService.detachPtlOrderToLocation(body = body)
        ...
        tempInst.enqueue(object : Callback<Any?> {
            override fun onResponse(call: Call<Any?>, response: Response<Any?>) {
                ...
                try {
                    val respObj = moshi.adapter(ApiResponse::class.java).fromJsonValue(resp)
                    if (respObj == null) {
                    ...
                    sendEvent(
                        respObj.details,
                        if (respObj.result == RESULT_OK) SnackBarType.SUCCESS else SnackBarType.ERROR
                    )
                } catch (ex: Exception) {
                    sendEvent(
