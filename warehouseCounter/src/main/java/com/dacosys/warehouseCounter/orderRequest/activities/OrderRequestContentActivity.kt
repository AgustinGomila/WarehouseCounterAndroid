...
import com.dacosys.warehouseCounter.confirmStatus.ConfirmStatus.CREATOR.confirm
import com.dacosys.warehouseCounter.confirmStatus.ConfirmStatus.CREATOR.modify
...
import com.dacosys.warehouseCounter.itemRegex.`object`.ItemRegex.CREATOR.RegexResult
...
import com.dacosys.warehouseCounter.orderRequest.`object`.*
...
class OrderRequestContentActivity :
    AppCompatActivity(),
    OrcAdapter.DataSetChangedListener,
    OrcAdapter.EditQtyListener,
    OrcAdapter.EditDescriptionListener,
    Scanner.ScannerListener,
    SwipeRefreshLayout.OnRefreshListener,
    Rfid.RfidDeviceListener,
    CheckCode.CheckCodeEnded {
    ...
    override fun onCheckCodeEnded(orc: OrderRequestContent?, itemCode: ItemCode?) {
        ...
        if (orc != null) {
            ...
            if (binding.requiredDescCheckBox.isChecked &&
                orc.item?.itemDescription == getContext().getString(R.string.unknown_item)
            ) {
    ...
    private fun setQty(orc: OrderRequestContent) {
        if (lastRegexResult != null) {
            ...
            if (qty == null)
                makeText(this, getString(R.string.null_quantity_in_regex), ERROR)
    ...
    private var tempTitle = getContext().getString(R.string.count)
    ...
    private fun saveBundleValues(b: Bundle) {
        if (orcAdapter != null) {
            ...
            b.putIntegerArrayList(
                "checkedIdArray",
                orcAdapter?.getAllCheckedAsInt() ?: ArrayList()
            )
        }
    ...
    private fun loadDefaultValues() {
        ...
        when (Statics.prefsGetInt(Preference.scanModeCount)) {
            1 -> {
                partialBlock = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (orderRequest != null) {
            if (orderRequest!!.description.isNotEmpty()) {
                tempTitle =
                    "${getContext().getString(R.string.count)}: ${orderRequest!!.description}"
            }
            ...
            if (orderRequest!!.orderRequestedType != null) {
                binding.orTypeTextView.text = when (orderRequest!!.orderRequestedType) {
                    ...
                    else -> getContext().getString(R.string.counted)
                }
            }
        }
        ...
        binding.requiredDescCheckBox.setOnCheckedChangeListener { _, isChecked ->
            Statics.prefsPutBoolean(Preference.requiredDescription.key, isChecked)
        }
        ...
        binding.requiredDescCheckBox.isChecked =
            Statics.prefsGetBoolean(Preference.requiredDescription)
    ...
    private fun setPanels() {
        ...
        if (panelBottomIsExpanded) {
            ...
            if (panelTopIsExpanded) currentLayout.load(
                this,
                R.layout.order_request_activity_bottom_panel_collapsed
            )
        ...
        when {
            panelBottomIsExpanded -> binding.expandBottomPanelButton?.text =
                ...
                getContext().getString(R.string.scan_options)
        }
        ...
        when {
            panelTopIsExpanded -> binding.expandTopPanelButton?.text =
                ...
                getContext().getString(R.string.item_operations)
        }
    }
    ...
    private fun setTopPanelAnimation() {
        ...
        binding.expandTopPanelButton!!.setOnClickListener {
            ...
            if (panelBottomIsExpanded) {
                if (panelTopIsExpanded) nextLayout.load(
                    this,
                    R.layout.order_request_activity_top_panel_collapsed
                )
                else nextLayout.load(this, R.layout.order_request_activity)
            } else {
                if (panelTopIsExpanded) nextLayout.load(
                    this,
                    R.layout.order_request_activity_both_panels_collapsed
                )
    ...
    private fun setupPartial() {
        runOnUiThread {
            ...
            if (!partialBlock && !partial) {
                buttonText = getContext().getString(R.string.partial_auto)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_ffed65,
                    null
                )
                buttonTextColor = ResourcesCompat.getColor(
                    this.resources,
                    R.color.black,
                    null
                )
            } else if (partialBlock) {
                buttonText = getContext().getString(R.string.partial_locked)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_6b2737,
                    null
                )
                buttonTextColor = ResourcesCompat.getColor(
                    this.resources,
                ...
                buttonText = getContext().getString(R.string.partial_manual)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_teal,
                    null
                )
                buttonTextColor = ResourcesCompat.getColor(
                    this.resources,
    ...
    private fun setupDivided() {
        runOnUiThread {
            ...
            if (!dividedBlock && !divided) {
                buttonText = getContext().getString(R.string.without_divider)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_ffed65,
                    null
                ...
                buttonText = getContext().getString(R.string.divider_locked)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_6b2737,
                    null
                ...
                buttonText = getContext().getString(R.string.divider)
                buttonBackground = ResourcesCompat.getDrawable(
                    this.resources,
                    R.drawable.rounded_corner_button_teal,
                    null
        ...
        if ((orcAdapter?.count() ?: 0) <= 0) {
            ...
            builder.setPositiveButton(R.string.yes) { _, _ ->
                if (processCount(false))
                    finish() // Salir si no hay error al guardar
            }
    ...
    private fun setMultiplierButtonText() {
        runOnUiThread {
            binding.multiplierButton.text = String.format(
                getContext().getString(R.string.multiplier_x),
                Statics.prefsGetInt(Preference.scanMultiplier)
            )
        }
    }
    ...
    private fun selectItemDialog() {
        if (!rejectNewInstances) {
            ...
            val intent =
                Intent(getContext(), ItemSelectActivity::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP

            intent.putExtra("title", getContext().getString(R.string.select_item))
    ...
    private fun showLogDialog() {
        if (!rejectNewInstances) {
            ...
            val intent =
                Intent(getContext(), LogContentActivity::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP

            intent.putExtra("title", getContext().getString(R.string.count_log))
    ...
    private fun setQtyManually(orc: OrderRequestContent) {
        val multi =
            if (itemCode == null) Statics.getMultiplier().toDouble() else itemCode?.qty ?: 0
        ...
        try {
            if (Statics.demoMode) {
                if ((orcAdapter?.count ?: 0) >= 5) {
                    val res =
                        getContext().getString(R.string.maximum_amount_of_demonstration_mode_reached)
                    makeText(binding.root, res, ERROR)
                    android.util.Log.e(this::class.java.simpleName, res)
                    return false
                }
            ...
            for (a in 0 until (orcAdapter?.count ?: 0)) {
                ...
                if (orc != null &&
                    orc.item?.itemId == tempOrc.item?.itemId
                ) {
                    setQtyCollected(
                        orc = orc,
                        qty = tempOrc.qty?.qtyCollected ?: 0.toDouble(),
                        manualAdd = true
                    )
                    return true
                }
    ...
    private fun showDialogForItemDescription(orc: OrderRequestContent) {
        if (!rejectNewInstances) {
            ...
            val intent =
                Intent(getContext(), EnterCodeActivity::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP

            intent.putExtra("title", getContext().getString(R.string.enter_description))
            intent.putExtra("hint", getContext().getString(R.string.item_description_hint))
            ...
            try {
                if (data != null) {
                    ...
                    if (it.resultCode == RESULT_OK) {
                        ...
                        if ((item.itemDescription.isEmpty() || item.itemDescription == getContext().getString(
    ...
    private fun lotCodeDialog(orc: OrderRequestContent) {
        if (!rejectNewInstances) {
            ...
            intent.putExtra("title", getContext().getString(R.string.enter_lot_code))
            intent.putExtra("hint", getContext().getString(R.string.lot_code))
            intent.putExtra("orc", orc)
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            ...
            try {
                if (it?.resultCode == RESULT_OK && data != null) {
                    ...
                    val orc = Parcels.unwrap<OrderRequestContent>(
                        data.getParcelableExtra("orderRequestContent")
                    )
    ...
    private fun multiplierDialog() {
        if (!rejectNewInstances) {
            ...
            val multiplier = Statics.prefsGetInt(Preference.scanMultiplier)
            val intent =
                Intent(getContext(), MultiplierSelectActivity::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_SINGLE_TOP
            intent.putExtra("title", getContext().getString(R.string.select_multiplier))
    ...
    private fun setDescription(
        ...
        if (updateDb) {
            ...
            if (item == null) {
                val res =
                    getContext().getString(R.string.an_error_occurred_while_updating_the_description_of_the_item_in_the_database)
                makeText(binding.root, res, ERROR)
                android.util.Log.e(this::class.java.simpleName, res)
                return
            }
    ...
    private fun setQtyCollected(
        ...
        val multi: Long = when {
            manualAdd -> 1L
                ...
                when (itemCode) {
                    null -> Statics.getMultiplier().toLong()
            ...
            when (requestCode) {
                REQUEST_EXTERNAL_STORAGE -> {
                    ...
                    if (grantResults.isEmpty() || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                        makeText(
                            this,
                            getString(R.string.cannot_write_images_to_external_storage),
                            ERROR
                        )
                    } else {
    ...
    override fun scannerCompleted(scanCode: String) {
        ...
        if (divided) {
            val splitCode =
                scanCode.split(Statics.divisorChar().toRegex()).dropLastWhile { it.isEmpty() }
            if (splitCode.any()) {
