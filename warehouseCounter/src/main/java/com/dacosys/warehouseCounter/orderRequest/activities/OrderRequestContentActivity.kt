...
class OrderRequestContentActivity : AppCompatActivity(), OrcAdapter.DataSetChangedListener,
    ...
    private fun loadDefaultValues() {
        ...
        log = Log(or.clientId,
            ...
            or.description,
            android.text.format.DateFormat.format("yyyy-MM-dd hh:mm:ss", System.currentTimeMillis())
                .toString(),
            ArrayList())
    ...
    private fun setPanels() {
        ...
        if (panelBottomIsExpanded) {
            ...
            if (panelTopIsExpanded) currentLayout.load(this,
    ...
    private fun setupPartial() {
        runOnUiThread {
            ...
            if (!partialBlock && !partial) {
                ...
                buttonBackground = ResourcesCompat.getDrawable(this.resources,
    ...
    private fun setMultiplierButtonText() {
        runOnUiThread {
            binding.multiplierButton.text =
                String.format(context().getString(R.string.multiplier_x),
                ...
                for (i in orcsToRemove) {
                    if (i.item != null && i.qty != null) {
                        ...
                        if (variationQty != 0.toDouble()) {
                            log!!.content.add(LogContent(itemId = itemObj.itemId,
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
            ...
            try {
                if (it?.resultCode == RESULT_OK && data != null) {
                    ...
                    try {
                        addOrc(OrderRequestContent(item = Item(item),
                            lot = null,
    ...
    private fun processCount(completed: Boolean): Boolean {
        ...
        tOrderRequest.finishDate =
            if (tempCompleted) android.text.format.DateFormat.format("yyyy-MM-dd hh:mm:ss",
                ...
                if (data != null) {
                    ...
                    if (it.resultCode == RESULT_OK) {
                        ...
                        if ((item.itemDescription.isEmpty() || item.itemDescription == context().getString(
                                R.string.unknown_item)) && binding.requiredDescCheckBox.isChecked
                        ) {
                            orcAdapter?.remove(orc)
                        }
                    }
            ...
            when (requestCode) {
                REQUEST_EXTERNAL_STORAGE -> {
                    ...
                    if (grantResults.isEmpty() || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                        ...
                        val error = !Statics.writeJsonToFile(v = binding.root,
                            filename = orFileName,
                            value = orJson,
                            completed = tempCompleted)
                        if (!error) finish()
    ...
    override fun scannerCompleted(scanCode: String) {
        ...
        if (divided) {
            val splitCode =
                scanCode.split(settingViewModel().divisionChar.toRegex())
