...
import com.dacosys.imageControl.network.webService.moshi.Document
...
class OrderRequest : Parcelable, JsonSerializer<OrderRequest>, JsonDeserializer<OrderRequest> {
        ...
        if (src != null) {
            ...
            jsonObject.add(
                "orderRequestedType", OrderRequestType().serialize(
                    src.orderRequestedType, OrderRequestType::class.java, context
                )
            )
            ...
            for ((index, cont) in src.content.withIndex()) {
                val jsonCont = OrderRequestContent().serialize(
                    cont, OrderRequestContent::class.java, context
                )

                val orcJson = JsonObject()
                orcJson.add(
                    "content$index", jsonCont
                )
            ...
            // TODO: IMAGE CONTROL DOCUMENTS
            /// val jsonDocumentArray = JsonArray()
            /// for ((index, doc) in src.docArray.withIndex()) {
            ///     val jsonCont = Document().serialize(
            ///         doc, Document::class.java, context
            ///     )

            ///     val orcJson = JsonObject()
            ///     orcJson.add(
            ///         "document$index", jsonCont
            ///     )

            ///     jsonDocumentArray.add(orcJson)
            /// }
            /// jsonObject.add(
            ///     "document", jsonDocumentArray
            /// )

            // LOG
            jsonObject.add(
                "log", Log().serialize(
                    src.log, Log::class.java, context
                )
            )
        }
    ...
    override fun deserialize(
        ...
        for ((index, cont) in jsonObject.getAsJsonArray("content").withIndex()) {
            if (cont is JsonObject) {
                ...
                if (c != null) {
                    content.add(
                        context.deserialize(
                            c, OrderRequestContent::class.java
                        )
                    )
                }
        ...
        for ((index, cont) in jsonObject.getAsJsonArray("document").withIndex()) {
            if (cont is JsonObject) {
                ...
                document.add(
                    context.deserialize(
                        Gson().fromJson(c.toString(), JsonElement::class.java), Document::class.java
                    )
                )
            }
        }
        ...
        val log = context.deserialize<Log>(
            jsonObject.get("log"), Log::class.java
        )
        ...
        or.orderRequestedType = context.deserialize(
            orderRequestedType, OrderRequestType::class.java
        )
    ...
    constructor(parcel: Parcel) : this() {
        ...
        content = parcel.createTypedArrayList(OrderRequestContent) as ArrayList<OrderRequestContent>
    ...
    override fun equals(other: Any?): Boolean {
        return if (other !is OrderRequest) {
            ...
            equals(this.orderRequestId, other.orderRequestId) && equals(
                this.filename.substringAfterLast('/'), other.filename.substringAfterLast('/')
            )
        }
    }
    ...
    companion object CREATOR : Parcelable.Creator<OrderRequest> {
        ...
        fun fromJson(json: String): OrderRequest? {
            ...
            val gSon = GsonBuilder().registerTypeAdapter(
                OrderRequest::class.java, OrderRequest()
            ).registerTypeAdapter(
                object : TypeToken<ArrayList<OrderRequestContent>>() {}.type, OrderRequestContent()
            ).registerTypeAdapter(
                Log::class.java, Log()
            ).registerTypeAdapter(
                object : TypeToken<ArrayList<LogContent>>() {}.type, LogContent()
            ).registerTypeAdapter(
                object : TypeToken<ArrayList<Document>>() {}.type, Document()
            ).registerTypeAdapter(
                object : TypeToken<ArrayList<DocumentContent>>() {}.type, DocumentContent()
            ).registerTypeAdapter(
